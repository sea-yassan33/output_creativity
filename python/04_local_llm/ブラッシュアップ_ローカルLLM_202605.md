...
# 「Gemma3」シリーズとクラウド型フラッグシップ「Gemini2.5-Flash」の性能

| 項目 | Gemma 3: 270M | Gemma 3: 1B | Gemini 2.5 Flash |
| --- | --- | --- | --- |
| **モデルタイプ** | オープンウェイト（ローカル） | オープンウェイト（ローカル） | クラウドAPI（マネージド） |
| **パラメータ数** | 2.7億 | 10億 | 非公開（中規模） |
| **得意なタスク** | 特定タスクの推論、単純な要約 | 指示への従順性、論理的思考 | 複雑な推論、マルチモーダル処理 |
| **コンテキスト長** | 32k tokens | 32k tokens | 1M tokens (100万) |
| **マルチモーダル** | 非対応（テキストのみ） | 非対応（テキストのみ） | 対応（画像、動画、音声、PDF） |
| **知識量・正確性** | 限定的（補助ツール向き） | 一般的（チャット・軽量開発） | 非常に高い（RAGや大規模分析） |
| **ベンチマーク例** | MMLU-Lite: ~34% (参考値) | MMLU-Lite: 34.2% | MMLU-Lite: ~88% |
